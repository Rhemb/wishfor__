        ...
        <div className="edit-page-container">
            ...
            <div>
                <form className="form-container row g-3 mt-5 d-flex flex-column justify-content-center align-items-center" onSubmit={updateItemHandler}>
                    <h1 className="d-flex justify-content-center mb-5" style={{fontSize: "30px"}}>Edit Item</h1>
                    <div className="row mb-3 d-flex flex-row needs-validation">
                        <div className="col-md-6 d-flex flex-column">
                            <label className="form-label " htmlFor="itemName">Item Name</label>
                            <input className="form-control" type="text" name="itemName" id="itemName" value={itemName} onChange={e => setItemName(e.target.value)} required></input>
                            { itemName.length < 2 ? <p className="text-danger">Must be 2 or more characters</p> : null }
                        </div>
                        <div className="col-md-6 d-flex flex-column needs-validation">
                            <label className="form-label" htmlFor="link">Link</label>
                            <input className="form-control" type="text" name="link" id="link" value={link} onChange={e => setLink(e.target.value)} required></input>
                            { link.length === 0 ? <p className="text-danger">Please include a valid link</p> : null }
                        </div>
                    </div>
                    <div className="row d-flex flex-row align-items-center">
                        <div className="col d-flex flex-column">
                            ...
                            { quantity <= 0 ? <p className="text-danger">Quantity must be greater than 0</p> : null }
                        </div>
                        <div className="col d-flex flex-column">
